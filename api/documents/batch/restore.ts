...
import { batchRestoreDocumentsFromTrash } from '../../../server/services/trash/documentTrashService.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const result = await batchRestoreDocumentsFromTrash(auth.ctx, auth.user, documentIds);
    ...
    for (const row of result.results.filter((r) => r.ok)) {
      await emitTrackingEvent(
        auditCtx,
        {
          ...
          description: 'Documento restaurado da lixeira (lote).',
