...
export function AuditSummaryCards({ overview, loading, showPending = true }: AuditSummaryCardsProps) {
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      {visibleCards.map(({ key, label, icon, accent, bg }) => (
        <Card key={key} className="border-doqyn-border-subtle bg-doqyn-surface/90 shadow-none">
          <CardContent className="flex items-center gap-3 p-4">
            <div className={cn('flex h-10 w-10 items-center justify-center rounded-lg', bg)}>
              <Icon name={icon} size={ICON_SIZE.xs} className={accent} />
            </div>
