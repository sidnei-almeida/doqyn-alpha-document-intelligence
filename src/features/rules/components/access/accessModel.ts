...
/** Dado, não frase: quem mostra o motivo é a tela, no idioma dela. */
export type SimulationResult =
  | { sees: true; via: 'admin' }
  | { sees: true; via: 'group'; groupName?: string }
  | { sees: false };
...
export function describeMemberGroups(member: CompanyMember, groups: Group[]): string {
  if (isMemberAdmin(member)) return i18n.t('rules:simulateAccessBar.memberAdmin');
  const names = member.groupIds
    .map((id) => groups.find((g) => g.id === id)?.name)
    .filter(Boolean) as string[];
  if (names.length === 0) return i18n.t('rules:simulateAccessBar.memberNoGroups');
  return i18n.t('rules:simulateAccessBar.memberGroups', { groups: names.join(', ') });
}
