...
async function ensureRegistryIndexes() {
  ...
  const { ensureDocumentSignatureRequestsIndexes, ensureDocumentSignaturesIndexes } =
    await import('./documentSignatureIndexes.js');
  ...
  const { ensureExternalEmailOutboxIndexes } = await import('./externalEmailOutboxIndexes.js');
  await ensureExternalEmailOutboxIndexes();
}
...
async function main() {
  ...
  // Mantém o efeito colateral (semear o tenant de dev); o documento em si não é mais necessário,
  // porque os nomes de coleção deixaram de depender do tenant.
  ...
  const collectionNames = resolveSharedCollections();
  ...
  console.log('Totais no database (sem seed de governança):', {
    database,
