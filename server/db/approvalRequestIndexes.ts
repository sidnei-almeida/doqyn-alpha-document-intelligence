...
export const APPROVAL_REQUEST_INDEXES: IndexDescription[] = [
  ...
  /**
   * Um pedido pendente por (tipo, assunto, **quem pediu**).
   *
   * O requerente faz parte da chave: sem ele, o segundo usuário a pedir download do mesmo
   * documento colidiria com o primeiro e ficaria sem conseguir pedir. E o filtro exige
   * `subject.documentId`: pedido de envio não tem documento ainda, e o Mongo indexa campo ausente
   * como `null` — dois envios pendentes colidiriam entre si.
   */
  {
    key: { tenantId: 1, kind: 1, 'subject.documentId': 1, 'requestedBy.userId': 1 },
    unique: true,
    name: 'tenant_kind_document_requester_pending_unique',
...
/**
 * Índice único da primeira versão, sem o requerente na chave.
 *
 ...
 */
const SUPERSEDED_INDEXES = ['tenant_kind_document_pending_unique'];
